...
import csv
# Libreria para manejar datos de tipo datetim
import datetime

# Clase para almacenar los incidentes
class Incidente:
  def _init_(self,FECHA,CASOS,MUERTES, PAIS):
    self.FECHA = FECHA
    self.CASOS = CASOS
...
with open('AnálisisCOVID.csv') as archivo_csv:
  ...
  for linea_datos in lector_csv:
    if contador_lineas == 0:
      # Si es la primera línea, muestro los nombres de campo y no guardo nada
      # en la lista.
      print(f'Los nombres de columna son {", ".join(linea_datos)}')
        ...
        objeto_temporal = Incidente({linea_datos[0]},{linea_datos[1]}, {linea_datos[2]},{linea_datos
